...
## STANDING STRATEGY (user directive, 2026-07-12)
**Target the HIGHEST-POPULARITY search terms in every country** — not just easy wins. Per-country popularity sources, in order of preference: (1) Astro DB where it has niche coverage (US mainly), (2) **Apple Search Hints API** as the local-popularity proxy everywhere else: `https://search.itunes.apple.com/WebObjects/MZSearchHints.woa/wa/hints?clientApplication=Software&term=<prefix>` with header `X-Apple-Store-Front: <storefrontID>-1,29` — the ORDER of returned suggestions reflects local search popularity for that prefix. Harvest with localized niche seed prefixes (manifest/vision/gratitude/369/angel/affirmation roots per language; see aso/bonus_keywords.json for translations), aggregate, and treat top-order suggestions as the country's most-popular terms. Tooling: `aso/hints_harvester.py` (build/maintain it). The optimization loop is: harvest per-country top terms → rank-check us on them → drive title/subtitle/keyword-field changes toward the biggest terms we can plausibly win, accepting harder fights on the biggest terms per the user's directive.
